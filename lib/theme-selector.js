...
module.exports = class ThemeSelector {
  constructor() {
    ...
    this.selectList = lumine.workspace.buildSelectList({
      ...
      confirmAction: "theme-selector:use-selected-theme-pack",
      additionalActionCommands: [
    ...
    this.subscriptions = new CompositeDisposable(
      ...
      // Registered in the package's own namespace on the list element: the
      // item-actions list derives its rows — label, description,
      // keybinding — from this registration and the keymap. Previewing used
      // to happen on every selection change, which meant a walk down the list
      // repainted the whole window per row; it is a deliberate act now.
      lumine.commands.add(this.selectList.element, {
        "theme-selector:use-selected-theme-pack": {
          ...
          didDispatch: () => this.confirm(this.selectList.getSelectedItem()),
        },
        "theme-selector:preview": {
          description: "Apply the selected pack without closing, so it can be seen in place.",
          didDispatch: () => this.preview(this.selectList.getSelectedItem()),
        },
      }),
    );
  }
